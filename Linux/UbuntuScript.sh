...
update(){
	...
	"Debian"|"Ubuntu")
		...
		killall firefox
		wait
		sudo apt --purge --reinstall install firefox -y
		wait
...
echo "$LogTime uss: [$UserName]# Installing Craklib..." >> output.log
	apt install libpam-cracklib || yum install libpam-cracklib
	wait
...
echo "$LogTime uss: [$UserName]# Checking for ssh..." >> output.log
	dpkg -l | grep openssh-server >> output.log
        	...
        	then
                	...
                	if [ $a = n ];
                	then
                        	apt autoremove -y --purge openssh-server ssh >> output.log
...
echo "$LogTime uss: [$UserName]# Installing and securing SSH now..." >> output.log
                        	apt install -y openssh-server ssh >> output.log
				wait
							sed -i 's/LoginGraceTime .*/LoginGraceTime 60/g' /etc/ssh/sshd_config
                        	sed -i 's/PermitRootLogin .*/PermitRootLogin no/g' /etc/ssh/sshd_config
                        	sed -i 's/Protocol .*/Protocol 2/g' /etc/ssh/sshd_config
...
##Looks for apache web server
	...
	then
        	...
        	if [ $a = n ];
        	then
      	        	apt autoremove -y --purge apache2 >> output.log
			else
...
##Looks for john the ripper
	...
	then
        	...
        	apt autoremove -y --purge john >> output.log
...
##Look for HYDRA
	...
	then
		echo "HEIL HYDRA"
		apt autoremove -y --purge hydra >> output.log
	else
...
##Looks for nginx web server
	...
	then
        	...
        	apt autoremove -y --purge nginx >> output.log
	else
...
##Looking for VNC
	...
	then
		...
		if [ $a = y ]
		then
			apt autoremove -y --purge x11vnc tightvncserver 
		##else
			##Configure VNC
		fi
	else
...
##Looking for NFS
	...
	then	
		...
		if [ $a = 0 ]
		then
			apt autoremove -y --purge nfs-kernel-server
		##else
			##Configure NFS
		fi
	else
...
##Looks for sendmail and postfix
	...
	then
		...
		apt autoremove -y --purge postfix sendmail
	else
...
#			apt autoremove --purge netcat netcat-openbsd netcat-traditional
#else
